#!/usr/bin/env bash
# render-site.sh <site-dir> <dist-dir> <host> <usign-fpr> <apk-fpr>
...
for f in "$dist/index.html" "$dist/install.sh"; do
  sed -i.bak \
    ...
    -e "s|__APK_FPR__|$apk_fpr|g" \
    -e "s/__PACKAGES__/$pkgs/g" \
    "$f" && rm -f "$f.bak"
done
if grep -rl '__FEED_HOST__\|__USIGN_FPR__\|__APK_FPR__\|__PACKAGES__' "$dist"; then
